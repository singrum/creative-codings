...
const works: {
  ...
  {
    path: "https://singrum.github.io/r3f-note/#/GoogleSearchSong",
      ...
      <div className="flex-1 flex min-h-0">
              ...
              {tags.map((e, i) => (
                <React.Fragment key={i}>
                  <div
                    ...
                    onClick={() => setChecked({ ...checked, [e]: !checked[e] })}
                  >
                    {e}
                  </div>
                </React.Fragment>
        ...
        <div className="flex-1 min-h-0 p-5 overflow-auto">
          <div className="flex justify-between mb-5">
            <div className="text-3xl font-thin tracking-wide">프로젝트</div>
            {/* <div className="p-2 rounded-full hover:bg-accent cursor-pointer w-10 h-10">
              <Filter className="w-6 h-6" />
            </div> */}
          </div>
          <ul className="w-full flex flex-wrap gap-y-4 sm:gap-x-[calc(100%-((100%/2-6px)*2))] md:gap-x-[calc(100%-((100%/3-5px)*3))] lg:gap-x-[calc(100%-((100%/4-4px)*4))]">
            {(Object.values(checked).every((e) => e === false)
              ? works
              : works.filter((e) => e.tag.some((t) => checked[t]))
            ).map((e, i) => (
              <React.Fragment key={i}>
                <li className="sm:w-[calc(100%/2-6px)] md:w-[calc(100%/3-10px)] lg:w-[calc(100%/4-12px)] flex flex-col gap-2">
                  <div className="text-muted-foreground text-xs">
                    {e.date.format("YYYY년 MM월")}
                  </div>
                  <img
                    src={e.imgSrc}
                    className={`object-cover aspect-video rounded-lg w-full bg-cover bg-center cursor-pointer hover:ring-primary hover:ring-1`}
                    onClick={() => window.open(e.path)}
                  />
                  <div
                    className="text-md cursor-pointer hover:text-primary hover:underline"
                    onClick={() => window.open(e.path)}
                  >
                    {e.name}
                  </div>
